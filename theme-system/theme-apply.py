#!/usr/bin/env python3
"""
Apply a helix theme across helix, kitty, tmux, and yazi (via inherited terminal palette).
...
Writes:
    ...
    ~/.config/tmux/theme-current.conf    — tmux status/pane-border/message styles
    ~/.config/helix/config.toml          — updates `theme = "..."`

Signals:
    kitty @ set-colors --all --configured  (live palette swap)
    tmux source-file (only if a server is already running)
...
TMUX_THEME_OUT = HOME / ".config/tmux/theme-current.conf"
...
def derive_tmux_style(palette):
    """tmux is a multiplexer, not a terminal emulator — it has no ANSI-16
    palette of its own (kitty supplies that). It only needs UI-chrome colors:
    status bar, pane borders, message line, copy-mode selection highlight."""
    bg = palette["background"]
    fg = palette["foreground"]
    accent = palette["color4"]  # blue — active window/pane highlight
    muted = palette["color8"]  # bright black — inactive window text
    border = palette["selection_background"]  # subtle divider, not full contrast
    sel_bg = palette["selection_background"]
    sel_fg = palette["selection_foreground"]
    return {
        "status-style": f"bg={bg},fg={fg}",
        "window-status-style": f"fg={muted}",
        "window-status-current-style": f"bg={accent},fg={bg}",
        "pane-border-style": f"fg={border}",
        "pane-active-border-style": f"fg={accent}",
        "message-style": f"bg={accent},fg={bg}",
        "message-command-style": f"bg={accent},fg={bg}",
        "mode-style": f"bg={sel_bg},fg={sel_fg}",
        "display-panes-colour": border,
        "display-panes-active-colour": accent,
    }


def write_tmux_conf(palette):
    TMUX_THEME_OUT.parent.mkdir(parents=True, exist_ok=True)
    style = derive_tmux_style(palette)
    lines = ["# generated by theme-apply.py — do not edit\n"]
    for k, v in style.items():
        lines.append(f'set -g {k} "{v}"\n')
    TMUX_THEME_OUT.write_text("".join(lines))
...
def reload_tmux():
    if not shutil.which("tmux"):
        return
    # Only reload if a server is already running — otherwise `source-file`
    # would spawn a throwaway server just to set options nothing will see.
    check = subprocess.run(["tmux", "list-sessions"], capture_output=True)
    if check.returncode != 0:
        return
    subprocess.run(["tmux", "source-file", str(TMUX_THEME_OUT)], check=False, capture_output=True)
